...
This stack deploys the Hotel Booking Agent using Amazon Bedrock AgentCore.
"""
...
class HotelBookingAgentStack(Stack):
    """CDK Stack for Hotel Booking Agent using AgentCore Runtime"""
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        self.tool_name = agentcore_context.get("tool-name", "hotel_booking_agent")
...
def handler(event, context):
    if event['RequestType'] in ['Create', 'Update']:
        ...
        secret_value = {'user_pool_id': user_pool_id, 'client_id': client_id, 'username': username, 'password': password, 'discovery_url': discovery_url}

        try:
            secrets.update_secret(SecretId=secret_name, SecretString=json.dumps(secret_value))
        except secrets.exceptions.ResourceNotFoundException:
            secrets.create_secret(Name=secret_name, SecretString=json.dumps(secret_value), Description=f'Cognito credentials for {secret_name.split("/")[0]}')
...
"""),
    ...
    def _create_agentcore_runtime(self) -> agentcore.Runtime:
        ...
        agent_runtime_artifact = agentcore.AgentRuntimeArtifact.from_asset(
            str(agent_path), platform=ecr_assets.Platform.LINUX_ARM64
        )

        runtime = agentcore.Runtime(
            self,
            "HotelBookingAgentRuntime",
            runtime_name=self.tool_name,
            agent_runtime_artifact=agent_runtime_artifact,
            execution_role=self.agentcore_role,
            protocol_configuration=agentcore.ProtocolType.HTTP,
            description="Hotel Booking Agent with Memory Support",
    ...
    def _create_outputs(self) -> None:
        ...
        CfnOutput(self, "AgentCoreRoleArn", value=self.agentcore_role.role_arn, description="IAM Role ARN")
        ...
        CfnOutput(
            ...
            value=f"Hotel Booking Agent deployed successfully. Runtime: {self.tool_name}",
    ...
    def _apply_cdk_nag_suppressions(self) -> None:
            ...
            [
                {
                    ...
                    "appliesTo": [
                        ...
                        "Resource::arn:aws:bedrock-agentcore:<AWS::Region>:<AWS::AccountId>:workload-identity-directory/default/workload-identity/hotel_booking_agent-*",
        ...
        NagSuppressions.add_resource_suppressions(
            self.user_pool,
            [
                {"id": "AwsSolutions-COG1", "reason": "Development/test user pool."},
                {"id": "AwsSolutions-COG2", "reason": "MFA not required for development."},
                {"id": "AwsSolutions-COG3", "reason": "Advanced security not required for development."},
            ],
        )
            ...
            [
                {
                    ...
                    "reason": "Lambda basic execution role required.",
                    "appliesTo": [
                ...
                {
                    "id": "AwsSolutions-IAM5",
                    "reason": "Wildcard for Secrets Manager suffix.",
                    "appliesTo": [
                        "Resource::arn:aws:secretsmanager:<AWS::Region>:<AWS::AccountId>:secret:hotel_booking_agent/cognito/credentials-*"
                    ],
                },
            ],
        )
        ...
        NagSuppressions.add_resource_suppressions(
            self.update_secret_function, [{"id": "AwsSolutions-L1", "reason": "Python 3.12 is latest stable."}]
        )
        ...
        NagSuppressions.add_resource_suppressions_by_path(
            ...
            [
                {
                    "id": "AwsSolutions-IAM4",
                    "reason": "CDK provider requires managed policy.",
                    "appliesTo": [
                        "Policy::arn:<AWS::Partition>:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
                    ],
                }
        ...
        NagSuppressions.add_resource_suppressions_by_path(
            ...
            [
                {
                    "id": "AwsSolutions-IAM5",
                    "reason": "CDK provider requires wildcard.",
