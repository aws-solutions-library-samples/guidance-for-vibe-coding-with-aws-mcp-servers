...
Whether you're using a temporary AWS Event Account or working in your own AWS account, you **must enable access to the foundation models** within Amazon Bedrock via the **AWS Management Console**.

<Aside type="caution" title="Don't Skip this part">
You must explicitly enable access to third-party models before they can be used with the Amazon Bedrock service. Follow the steps in the [Amazon Bedrock User Guide](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access.html) to enable access to models from the providers below:

- Amazon (all models since they are first-party)
- Anthropic Claude models (**ONLY** enable Claude 3.5 Sonnet v2, Claude 3.7 Sonnet and Claude Sonnet 4)
- If you wish to use [cross-Region inference](https://docs.aws.amazon.com/bedrock/latest/userguide/cross-region-inference.html) as part of the workshop (and we suggest you do if the model supports it), then you'll need to enable the models in Bedrock not just in the Region in which you're working but also in a different Region. In this workshop, we have tested doing this in us-west-2 with the models also enabled in us-east-1 as the additional Region for inference.

You may also find the steps in [this introductory workshop](https://catalog.workshops.aws/building-with-amazon-bedrock/en-US/prerequisites/bedrock-setup) helpful.
</Aside>
...
<Aside type="note">
- **Depending on your development environment**, you can follow the configuration and verification steps for your tool of choice.
</Aside>
...
4. In the chat window, you will see an icon at near the bottom right (top right of the prompt box) that looks like a checklist, click the icon and then click *"create a new rule"*. Enter a name for your new rule such as "rule1". This will open a new tab in VS Code-Server with a file of that name.
    ...
    - AWS CDK MCP Server (awslabs.cdk-mcp-server) to build AWS CDK applications
