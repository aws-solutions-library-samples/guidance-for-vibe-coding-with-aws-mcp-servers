...
Remove all deployed AWS resources by destroying the CDK stacks in reverse order. The AgentCore agents and MCP server will be automatically destroyed as part of their respective stacks:

```bash title="CDK cleanup"
# Destroy the MCP Server stack (includes MCP server AgentCore runtime)
pnpm cdk destroy VibeCodingWorkshopMcpServer

# Destroy the Booking Agent stack (includes booking agent AgentCore runtime)
...
aws cloudformation describe-stacks --stack-name VibeCodingWorkshopMockApis 2>/dev/null || echo "✅ Mock APIs stack deleted"
aws cloudformation describe-stacks --stack-name VibeCodingWorkshopBookingAgent 2>/dev/null || echo "✅ Booking Agent stack deleted"
aws cloudformation describe-stacks --stack-name VibeCodingWorkshopMcpServer 2>/dev/null || echo "✅ MCP Server stack deleted"
```

### Step 4: Clean Up CDK Bootstrap (Optional)
