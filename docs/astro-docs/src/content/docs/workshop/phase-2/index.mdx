...
<Aside type="note" title="💡 Encountering Issues?">
Encountering issues? Consider using your AI coding assistant - this is part of the "vibe coding" experience!
</Aside>
...
### 🖥️ Frontend MCP Server Activity

**Activity**: Build a web frontend based on the interactive chat script

<Aside type="tip" title="Enhanced Visuals with Nova Canvas">
The AWS Frontend MCP Server can leverage the Nova Canvas MCP Server to generate custom graphics and visual elements (like hero banners, icons, or promotional images) that enhance your web interface with engaging imagery.
</Aside>
...
<Aside type="tip" title="Workshop Studio Users: Accessing Your Vite Dev Server">
If you're using an AWS-furnished VS Code environment, the Vite dev server (port 5173) isn't accessible by default. This helper script updates the security group to allow access from your IP address.

Run the script with your public IP address (find it at [ifconfig.me](https://ifconfig.me)):

```bash
./tools/workshop-studio/scripts/open-vite-access.sh YOUR_PUBLIC_IP
```

For detailed troubleshooting, see [Frontend Development Environment Setup](/faq/frontend-troubleshooting/#workshop-studio-environment-setup).
</Aside>
