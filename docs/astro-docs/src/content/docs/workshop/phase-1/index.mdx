...
1. **Configure Bruno Environment Files**
   ...
   Create your local Bruno environment files from templates with your deployed API URLs and keys:
   ...
   aws cloudformation describe-stacks --stack-name VibeCodingWorkshopMockApis --query "Stacks[0].Outputs"
   ```
   ...
   **Property Resolution environment:**
   ```bash title="Creating Bruno Environment file from template"
   # Copy template to create your local environment file
   cd packages/cdk-infra-python/tests/bruno/booking_mock_apis/environments
   cp PropertyResolution.bru.template PropertyResolution.bru
   
   # Edit the file with your API details
   code PropertyResolution.bru
   ```
   ...
   **Toxicity Detection environment:**
   ```bash title="Creating Bruno Environment file from template"
   # Copy template to create your local environment file
   cp ToxicityDetection.bru.template ToxicityDetection.bru
   
   # Edit the file with your API details
   code ToxicityDetection.bru
   ```
   ...
   **Reservations environment:**
   ```bash title="Creating Bruno Environment file from template"
   # Copy template to create your local environment file
   cp Reservations.bru.template Reservations.bru
   
   # Edit the file with your API details
   code Reservations.bru
   ```
...
2. **Set Up AWS Credentials**
   ...
   <Aside type="note" title="AWS Credentials Required">
   The API tests require these environment variables:
   - `AWS_ACCESS_KEY_ID`
   - `AWS_SECRET_ACCESS_KEY`
   - `AWS_SESSION_TOKEN` (for temporary credentials)

   **Workshop Studio**: 
   - Find credentials in your Workshop Studio dashboard
   ...
   **Self-Deployment - macOS/Linux/Git Bash**: 
   - Option 1: Use the helper script (recommended):
     ...
     cd ../..
     ```
   - Option 2: Set variables manually if AWS CLI is already configured
