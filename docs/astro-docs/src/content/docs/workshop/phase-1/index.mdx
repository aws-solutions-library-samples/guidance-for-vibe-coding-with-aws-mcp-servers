...
### 🧪 Testing the External Mock APIs

<Aside type="caution" title="Note for Self-Deployment Users">
**If you deployed using Option 2 (your own AWS account)**, you may see 403 Forbidden errors when running tests from your local machine. This is expected!

The APIs use `aws:SourceAccount` Resource Policies for security, which only allow requests from within the AWS account (not from local machines). This demonstrates real-world API security patterns.

**Workshop Studio users (Option 1)** won't see this issue since the environment runs within AWS.
</Aside>
...
1. **Configure Bruno Environment Files**
   ...
   Or check the AWS CloudFormation console for outputs and API Gateway console for key values.
   </Aside>
   ...
   **Reservations environment:**
   ...
   Replace the placeholder values with your deployed API details:
   ```js ins="YOUR_RESERVATIONS_API_ID" ins="YOUR_PROPERTY_API_ID" ins="YOUR_REGION" ins="YOUR_PROPERTY_RESOLUTION_API_KEY" ins="YOUR_RESERVATIONS_API_KEY" title="Reservations.bru"
   vars {
     apiUrl: https://YOUR_RESERVATIONS_API_ID.execute-api.YOUR_REGION.amazonaws.com/dev/api/v1
     apiKey: YOUR_RESERVATIONS_API_KEY
   ...
   <Aside type="note">
   The Reservations environment includes references to the Property Resolution API because the Setup tests dynamically populate hotel data before running reservation tests. The hotel placeholder values will be filled in automatically by the Setup tests.
   </Aside>

2. **Run API Tests**

   Navigate to the CDK infrastructure directory and run the test scripts:
   ...
   <Aside type="caution">
   If tests fail, double-check that your environment files have the correct API URLs and keys from your CDK deployment output.
   </Aside>
...
3. **Analyze Test Results**
   ...
   - **✅ Property Resolution**: How natural language queries like "chicago hotels" are processed using Amazon Location Services and return ranked results with `hotel_id` fields
   - **✅ Reservations**: The complete booking lifecycle from creation to cancellation, including complex filtering and pagination  
   - **✅ Toxicity Detection**: Content moderation with configurable thresholds and whitelisting for hotel-specific terms
